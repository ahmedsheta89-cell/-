...
import { PLATFORM_ADMIN_EMAIL } from '../../infrastructure/firebase/firebaseClient.ts';
...
export const FirebaseAuthModal: React.FC<FirebaseAuthModalProps> = ({
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (msg.includes('unauthorized-domain')) {
        setErrorMsg('نطاق العرض يحتاج تفعيل في Firebase Console، يمكنك استخدام التسجيل المباشر بالبريد الإلكتروني بالأسفل فهو يعمل فورياً.');
      } else if (!msg.includes('popup-closed-by-user')) {
        setErrorMsg('تعذر تسجيل الدخول بـ Google حالياً. يمكنك استخدام التسجيل المباشر بالبريد الإلكتروني بالأسفل.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleDirectAdminLogin = async () => {
    ...
    setLoading(true);
    try {
      await loginAsAdminDirectly();
      setSuccessMsg('تم تأكيد هوية المشرف العام (أحمد شتة) وتفعيل الصلاحيات بنجاح!');
      setTimeout(() => {
        onClose();
        setSuccessMsg(null);
      ...
      setErrorMsg('تعذر تفعيل بوابة المشرف.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (tab === 'LOGIN') {
        ...
        setSuccessMsg(`تم تسجيل الدخول بنجاح: ${u.displayName || u.email}`);
      } else {
        const u = await registerWithEmail(email, password, nameInput || 'طالب القرآن');
        setSuccessMsg(`تم إنشاء وتوثيق الحساب السحابي بنجاح: ${u.displayName || u.email}`);
      }
      setTimeout(() => {
        onClose();
        setSuccessMsg(null);
      ...
      if (msg.includes('invalid-credential') || msg.includes('wrong-password')) {
        ...
        setErrorMsg('هذا البريد مسجل مسبقاً، يرجى التبديل لتبويب تسجيل الدخول.');
      } else if (msg.includes('weak-password')) {
        setErrorMsg('يرجى كتابة كلمة مرور مكونة من 6 أحرف على الأقل.');
      } else {
        setErrorMsg('حدث خطأ أثناء معالجة الطلب، يرجى المحاولة مجدداً.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleSaveName = async (e: React.FormEvent) => {
    ...
    setSuccessMsg('تم حفظ وتحديث الاسم بنجاح في السحابة!');
    setTimeout(() => {
      setSuccessMsg(null);
      onClose();
    }, 1200);
        ...
        <div className="p-5 sm:p-6 space-y-4 overflow-y-auto flex-1 overscroll-contain">
          ...
          <div className="p-4 rounded-2xl bg-stone-50 border border-stone-200 space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="text-stone-500">حالة الاتصال السحابي:</span>
              <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full font-bold text-2xs bg-emerald-100 text-emerald-900 border border-emerald-200">
                ...
                <span>{user ? 'متصل سحابياً (Firebase Sync)' : 'زائر (غير مسجل)'}</span>
              </div>
            </div>

            <div className="flex items-center justify-between text-xs">
              <span className="text-stone-500">الحساب والاسم:</span>
              <span className="font-bold text-stone-900 font-mono">
                {user ? user.displayName || user.email : 'طالب جديد'}
              </span>
            </div>
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              <span className={`font-bold px-2 py-0.5 rounded-md text-3xs ${
                ...
                {isAdmin ? '👑 المشرف العام للمنصة (Super Admin)' : user ? 'طالب مسجّل وموثق' : 'طالب زائر'}
              </span>
            </div>
          </div>
            ...
            <div className="p-4 rounded-2xl bg-gradient-to-br from-amber-500/10 to-amber-600/15 border border-amber-300 text-amber-950 space-y-2.5">
              ...
              {onNavigateToAdmin && (
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-700 hover:to-amber-800 text-white text-xs font-bold transition-all shadow-xs flex items-center justify-center gap-2 cursor-pointer active:scale-95"
                >
          ...
          {tab === 'ADMIN' && !user && (
            <div className="p-4 rounded-2xl bg-amber-50 border border-amber-200 space-y-3">
              <div className="flex items-center gap-2 text-xs font-bold text-amber-950 font-arabic-heading">
                <Crown className="w-4 h-4 text-amber-600" />
                <span>دخول المشرف العام المعتمد للمنصة:</span>
              </div>
              <p className="text-2xs text-stone-600 leading-relaxed font-mono">
                {PLATFORM_ADMIN_EMAIL}
              </p>
              <p className="text-3xs text-stone-600 leading-relaxed">
                اضغط على الزر أدناه لتفعيل صلاحيات المشرف العام مباشرة والوصول للوحة التحكم:
              </p>
              <button
                onClick={handleDirectAdminLogin}
                disabled={loading}
                className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-700 hover:to-amber-800 text-white text-xs font-bold transition-all shadow-sm flex items-center justify-center gap-2 cursor-pointer active:scale-95 disabled:opacity-50"
              >
                <Crown className="w-4 h-4 text-amber-200" />
                <span>دخول مباشر كـ مشرف عام (أحمد شتة)</span>
              </button>
            </div>
          )}
          ...
          {!user && (
            <div className="space-y-3">
              ...
              <p className="text-3xs text-stone-500 text-center leading-relaxed">
                يدخل كل طالب بحسابه لحفظ درجاته وتلاواته، ويدخل المشرف بحسابه لإدارة المنصة ومتابعة الطلاب.
              </p>
            </div>
          )}
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="ahmed.sheta89@gmail.com أو بريد الطالب"
                    required
